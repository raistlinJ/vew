...
class WatchConnThread(QThread):
    ...
    def run(self):
        ...
        while(True):
            ...
            if self.status["connStatus"] == Connection.CONNECTING or self.status["disConnStatus"] == Connection.DISCONNECTING or self.status["refreshConnStatus"] == Connection.REFRESHING:
                ...
                for i in range(1,dots):
                    dotstring = dotstring + "."
                self.watchsignal.emit( "Trying to establish connection"+dotstring, self.status, None)
                dots = dots+1
                if dots > 4:
                    dots = 1
            else:
        ...
        if self.status["connStatus"] == Connection.CONNECTED:
            self.watchsignal.emit("Connection Established", self.status, True)
            return
        else:
            self.watchsignal.emit("Could not connect", self.status, True)
            return

class LoginConnectingDialog(QDialog):
    def __init__(self, parent, connName):
        logging.debug("LoginConnectingDialog(): instantiated")
        super(LoginConnectingDialog, self).__init__(parent)     
    ...
    def exec_(self):
        t = WatchConnThread(self.connName)
        t.watchsignal.connect(self.setStatus)
        t.start()
        result = super(LoginConnectingDialog, self).exec_()
