...
###Packager tests
    ...
    e.execute("packager export ExperimentData/sample/Experiments/sample.xml myresfile.res")
    res = e.execute("packager status")
    while res["writeStatus"] != PackageManageVBox.PACKAGE_MANAGE_COMPLETE:
        sleep(1)
...
###Connection tests
    ...
    e.execute("conns create ExperimentData/sample/Experiments/sample.xml")

    # sleep(10) #alternative, check status until connection manager is complete and idle
    e.execute("conns status")
    e.execute("conns remove ExperimentData/sample/Experiments/sample.xml")
    
    # sleep(10) #alternative, check status until connection manager is complete and idle
    e.execute("conns status")
    e.execute("conns open ExperimentData/sample/Experiments/sample.xml 1 1")

    #####---Create Experiment Test#####
    logging.info("Starting Experiment")
    e.execute("experiment create ExperimentData/sample/Experiments/sample.xml")
    ...
    logging.info("Starting Experiment")
    e.execute("experiment start ExperimentData/sample/Experiments/sample.xml")
    res = e.execute("experiment status")
    ...
    e.execute("experiment stop ExperimentData/sample/Experiments/sample.xml")
    res = e.execute("experiment status")
    ...
    e.execute("experiment remove ExperimentData/sample/Experiments/sample.xml")
    res = e.execute("experiment status")
